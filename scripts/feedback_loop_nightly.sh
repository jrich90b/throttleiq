...
mkdir -p "$REPORT_ROOT" "$EDIT_FEEDBACK_OUT_DIR" "$LANGUAGE_CORPUS_OUT_DIR" "$VOICE_FEEDBACK_OUT_DIR" "$OUTCOME_QA_OUT_DIR" "$VEHICLE_WATCH_QA_OUT_DIR" "$INBOUND_SHADOW_OUT_DIR" "$TONE_QUALITY_OUT_DIR" "$VOICE_CHARTER_OUT_DIR" "$AGENT_MANAGER_OUT_DIR" "$LOG_DIR"
