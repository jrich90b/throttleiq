...
function buildPrompt(task: AgentTask, claim: MdfClaimEntry, options: RunnerOptions): string {
  ...
  return [
    ...
    "   8c. REQUIRED: every uploaded file has a CATEGORY dropdown next to it that defaults to blank — you MUST set it BEFORE saving or the save fails with a database/foreign-key error. Set each invoice PDF's category to \"Invoice\" and each supporting .xlsx's category to \"Supporting Documentation\". Confirm NO uploaded file has a blank category before clicking Save for Later.",
