...
function runOnce(): Promise<number> {
  return new Promise(resolve => {
    ...
    child.on("error", err => {
      console.error("[mdf portal daemon] runner spawn failed:", err?.message ?? err);
      resolve(1);
    });
  });
}
...
async function tick() {
  ...
  try {
    ...
    if (code !== 0) console.warn(`[mdf portal daemon] runner exited with code ${code}`);
  } catch (err: any) {
    console.warn("[mdf portal daemon] runner failed:", err?.message ?? err);
  } finally {
    running = false;
  }
}
...
console.log(`[mdf portal daemon] polling ${apiBase} every ${Math.round(intervalMs / 1000)}s`);
void tick();
