...
## Twilio conversations: comprehend, never regex (Joe, hard rule, 2026-06-13)

- When a Twilio customer's intent is missed, fix the intent parser and add a replay fixture from that exact turn. Never add or broaden a conversational keyword/regex.
- When you reach for a regex to read what a customer *means*, stop — route it through the LLM intent parser instead.
...
- Migrated (fail-safe reply routers): `isScheduleContextStatusUpdateText`, `isDealerLocationQuestionText`, `isExplicitCustomerCallbackRequestText`, `isImmediateChatCallbackAvailabilityText`. Kept (fail-unsafe): `isWatchConfirmationIntentText` (watch creation), `hasCompetingActiveIntentText` (premature close), the regen callback no-response anti-suppression backstop, `isLienHolderInfoRequestText` (finance-doc todo + manual handoff), `isPaymentNumbersStatusQuestionText` (payments todo + handoff + escalation).
- Finance/pricing early-return guards, annotated in-code (`services/api/src/index.ts`): `isLienHolderInfoRequestText` + `isPaymentNumbersStatusQuestionText` are side-effect/handoff KEEPs; `isAffordabilityRideConfidenceObjectionText` is **KEEP** — re-classified 2026-06-22 by the de-tangle fail-direction test. It looks like a fail-safe reply-router at its index.ts reply sites, but the SAME detector is a fail-UNSAFE invariant guard in `shouldSuppressDispositionCloseout` (`transitionSafety.ts`): a false negative there lets the agent prematurely CLOSE a live, engaged lead. A detector feeding a fail-unsafe close-suppression guard can't be deleted/parsered away, so it stays deterministic (part of the ratchet KEEP-floor). The earlier "MIGRATE candidate" label only looked at the reply site.
...
Required order:
1) Define the parser's shape ONCE, as a **Zod** schema (Joe, 2026-08-10) — derive the JSON schema sent to the model, the TypeScript type, and the runtime validation of the result from that single definition. Do NOT hand-write a JSON-schema object beside a matching TS type: they drift silently and nothing fails when they do (the compiler trusts the type, the model follows the schema, and a value the code does not handle falls into the safe default unnoticed). Measured 2026-08-10: 82 hand-written schemas, 66 parsers, 68 copies of the same confidence clamp. Zod sits **behind** `requestStructuredJson`, never instead of it — that one choke point owns model fallback, the distillation capture and the truncation caps, and no library may take it over. Home is `services/api/src/domain/llmDraft.ts`, or its own `domain/<name>.ts` module when llmDraft sits at its size ceiling.
   - **This binds NEW parsers only.** Do not migrate the existing 66: nothing is broken, no bug has traced to schema/type drift, and a sweep would touch every comprehension path in the product to fix a problem that has not bitten. Convert one only when already editing it for another reason.
   - Measured before choosing Zod over the heavier options (86,510 parser calls / 7 days): **99.86% structured, 0.14% fallback, ZERO truncated outputs.** The retry/repair pitches of Instructor and BAML answer failure rates we do not have, and both — like the Vercel AI SDK — want to own `requestStructuredJson`. The 0.14% fallback path is the one place worth validating, and an auto-retry is still the WRONG fail direction there: our parsers return nothing and let existing behavior run, and pushing to force an answer inverts that.
...
- **Never silence (web-widget + used-no-price)**: a sales lead must always get at least an acknowledgment, even when the substantive answer must wait for staff. The `/public/widget/text-us` handler and `/conversations/:id/regenerate` path both fall back to an ack draft built from the requested vehicle / `conv.inventoryContext` ("Hi {first} — thanks for reaching out about the {year model}. Let me look into that for you and I'll text you right back.") whenever no buy/trade, requested-vehicle, handoff-ack, or orchestrator draft is produced. Routing to `manual_handoff` + a staff task is correct, but it must never replace a reply to the customer. Origin: Mike +17163686204 (2026-06-13) asked the price of a used 2013 Street Glide with no posted price and got dead air. Gate: `npm run web_text_widget:eval` pins both the live and regen fallbacks.
...
- **Hardship empathy (2026-06-18)**: when a customer discloses a personal hardship or serious situation (illness, injury, hospitalization, grief/loss, a family or financial emergency), the reply must LEAD with one short, genuine acknowledgment of THAT hardship before any business, and drop all scarcity/urgency/sales pressure ("moves quick", "won't last", "limited") — that is not the moment. Answer their actual request gently (e.g. how to leave a deposit / hold a bike) with no push to come in, and reassure there's no rush. This generalizes the financial-empathy rule to personal hardship. The LLM affect parser already flags this (`needsEmpathy`); the main draft prompt carries the instruction (gated on `needsEmpathy`), a deterministic backstop prepends "I'm really sorry to hear that." at the orchestrator finalize choke point (BOTH paths, `hardshipEmpathyAck.ts`, guarded against double-ack), and the proactive visit invite is suppressed on the same turn. Origin: Nicholas Braun +17166286477 (2026-06-17) disclosed a medical emergency / hospitalization while asking to hold a bike with a deposit; the draft replied "those limited runs move quick" and punted the deposit to staff (caught in suggest mode). Detection net: the tone scorer's `hardship_ack_missing` issue. Gate: `npm run hardship_empathy_ack:eval`.

## Cadence Template Bank Voice Rewrite (2026-06-11)
- All SMS cadence/variant banks (`FOLLOW_UP_MESSAGES`, test-ride, slot/no-slot/no-model variants, engaged maps, sell/trade banks, fresh-info/soft-exit, check-in fallbacks, anchor messages, early-promotion discovery, test-ride availability checkbacks) and `EMAIL_FOLLOW_UP_MESSAGES` rewritten to the Agent Voice Charter register: no banned filler, zero em-dashes in template banks, contractions, concrete offers (photos, numbers, time windows) instead of offers-to-help.
- **New guardrail:** any cadence template edit must preserve the string's outbound state-signal class — scheduling prompt (`draftHasSchedulingPrompt`), short-list ask (`outboundAsksForShortList`), media offer (`isMediaOfferFollowUpText`), preference clarifier (`isCadencePreferenceClarifierMessage`). Gate: `npm run cadence_template_voice:eval` extracts every bank from source, renders both label states, runs the voice-charter checker on each, and asserts signal classes. It also pins the detector regexes; if a detector changes, update the eval copy.
...
## Disengagement Taper (2026-06-13)
- A lead that never reached back must not be nudged through the full 13-step cadence. After `DISENGAGED_TAPER_AFTER_TOUCHES` (9, Joe's call) unanswered touches with zero customer inbound, the cadence emits ONE graceful close-out (`buildDisengagedCadenceCloseout`) and then ends. Origin: Michael Digiulio +17168660252 got 10 touches across SMS, email, and a voicemail without a single reply, still scheduled for more, with two byte-identical sends.
- "Engaged" for taper purposes means the CUSTOMER reached back — `customerEngagedWithCadence(conv)` counts any inbound message that isn't the originating `sendgrid_adf` web-lead form. Our own outbound (texts, emails, even an outbound call/voicemail) never marks a silent lead engaged. This also sidesteps the cosmetic `cadence.kind === "engaged"` mislabel (engaged/standard share `FOLLOW_UP_DAY_OFFSETS`, so the kind never changed the schedule anyway).
- Wiring: `shouldSendDisengagedCloseout(conv, sendingStep)` gates the close-out in three places — the live cadence tick (overrides `message`, skips dedupe rotation + context/personalization appends so it stays pristine), the email path (`emailMessage`), and `buildCadenceRegeneratedDraft` (so a staff regenerate of a stuck draft yields the close-out). `advanceFollowUpCadence` then sets `status: "completed"`, `stopReason: "disengaged_taper"`. Post-sale and long-term cadences are exempt. Gate: `npm run disengaged_taper:eval`.
...
## Call-Attempt Cadence (2026-06-13)
- A staff outbound call only "closes" a lead when the customer is actually REACHED. A voicemail / no answer must NOT resolve the follow-up — the lead stays on the same follow-up cadence and the open task shows the next attempt number ("2nd attempt", "3rd attempt") until reached. Origin: Merton Kreps +17165503586 — Joe called from the chat window, hit voicemail, and the lead stopped following up because the call looked handled.
- Two fixes in the recording handler (`/webhooks/twilio/voice` recording callback, ~index.ts 60630): (1) the call follow-up is closed via `markOpenCallTodosDoneForCompletedVoiceAttempt` ONLY when `contactedValue === "YES"`; a miss instead calls `registerMissedContactAttempt(conv)`, a reach calls `registerContactReached(conv)` (both up front so the finance-handoff todo can label the next attempt). (2) Voicemail detection hardened: `isLikelyVoicemailTranscript` now requires a "Customer:" line to carry real substance (>= 4 words, no voicemail phrasing) before it counts as a live conversation — a bare name fragment from the greeting ("Mert Platts.") no longer reads as a connected call. Regex also learned "your message for", "the person you are trying to reach", "press N", "google voice".
- `conv.contact = { attempts, reachedAt, lastAttemptAt, lastOutcome }`. `nextContactAttemptLabel(conv)` = `ordinalLabel(attempts + 1) + " attempt"`. Web shows an amber "Nth attempt" badge on the conversation's open-task card (derived from `conv.contact`, which flows through the `...conv` spread in GET /conversations/:id). Finance/handoff leads keep cycling via the existing `finance_no_contact` path (restarts the cadence + creates the attempt-labeled call todo). Gate: `npm run call_attempt_cadence:eval`; the `voice_call_followup` eval was re-pinned from "close including voicemail" to the reached-only contract.
- The fix is forward-looking: a lead whose past voicemail was mis-detected as contacted (engagement/contacted already set) won't retroactively reclassify — the next call attempt is handled correctly, or clear `conv.engagement`/`conv.contact` to reset.

## Web Deploy (artifact mode, 2026-06-13)
- The web app (`apps/web`, Next.js) runs on the box from a git checkout at `/home/ubuntu/throttleiq/apps/web` via `pm2` process `leadrider-web` (`next start --port 3000`). Deploys are NOT automatic — the box checkout lagged ~weeks of UI work before this was sorted.
- Deploy with `bash scripts/deploy_web_lightsail.sh` (defaults to host alias `lightsail`). It mirrors the API artifact pattern: build `.next` LOCALLY, then `git pull --ff-only` the box source + rsync the built `.next` (excluding `cache/`) + `pm2 restart leadrider-web`. **Never run `next build` on the prod box** — that build is what crushed it (see Deploy build resource guard).
- The script preflights and aborts unless: local HEAD == origin/main, the box checkout is a clean fast-forward of origin/main (no divergence), and `apps/web/package.json` + `package-lock.json` are unchanged since the box's commit (otherwise the box `node_modules` is stale — run `npm ci` on the box first). Build-time env (`NEXT_PUBLIC_*`) is left at defaults to match how the box was built; the client uses relative `/api`, server route handlers read runtime env preserved across `pm2 restart`.
...
## Quality Scorer Exclusions + Shadow Replay Discovery (2026-06-11)
...
- **Production lesson (2026-08-10):** a scorer that asks "did anything come back?" must read the EMAIL lane's draft too. On an SMS thread the agent's unsent draft is a message row (`provider: "draft_ai"`); on an email thread it is the `conv.emailDraft` FIELD and no row is written — so an answered email thread is indistinguishable from an ignored one. Haywood Kirkland +17166977040 (`HD.com Request a Quote` ADF, drafted **6 seconds** after arrival) failed the release gate TWICE the same morning: as the tone scorer's second `missing_response` (`toneMissingResponses` 2 > 1) and as the route watchdog's only actionable stuck turn (`freshStuckActionable` 1 > 0) — two of the day's three failures, one lead, agent correct. Fix: `hasPendingFirstTouchEmailDraft` (scoringExclusions.ts) consumed by `tone_quality_eval` (skip `email_draft_pending`) and `routeWatchdogClassification` (suppression `email_draft_pending`; the boolean is bound by the caller so the classifier still reads no text, same division of labour as `reaction_only`). **`emailDraft` carries no timestamp**, so the predicate is pinned to the only unambiguous shape — a FIRST TOUCH (one inbound, zero outbound); do not widen it, or a leftover draft would excuse a dropped follow-up. Verified on the live 833-conversation store: tone missing 2→0, watchdog actionable 1→0, exactly one row moved. Gate: `npm run scoring_exclusions:eval` + `npm run route_watchdog_stuck:eval`.
- **Production lesson (2026-08-10):** a customer who tells us NOT to write back is not a dropped turn. Robert Czechowski +17164808010 answered a seat question, then texted *"Please don't respond today it's you're day off"*; the agent correctly said nothing and was charged with a `missing_response`. `isExplicitNoReplyRequest` (scoringExclusions.ts) covers the negated directive ("don't respond/reply/text back", "no need to reply") and the noun form ("no response needed"). It is distinct from an ack, a closer, and an indefinite deferral — those are the customer describing THEIR next step; this is an instruction about OURS. Fail-safe via a length ceiling, a question-mark guard, and the deferral matcher's actionable-request guard, so "don't reply tonight, but send me the price tomorrow" still scores; the guard is allowed to over-block ("don't text me" trips its own `text me` token and stays graded). Gate: `npm run scoring_exclusions:eval`.

## Customer Vehicle Photo Share Routing (2026-06-11)
- Production case: Mustafa +17164368801 fulfilled staff's "send me pictures of bikes you like" ask on 6/10; the turn was classified `small_talk`, drafts asked discovery questions the photo already answered, and regenerate fell through to a stale cadence override about an unrelated sold unit.
- Parser-first per the standing rule: `customer_shared_vehicle_photo` added to the inbound reply action parser (union + strict schema enum + EXAMPLE P with the literal production turn + EXAMPLE Q distinguishing photo *requests*), deterministic fallback `detectCustomerVehiclePhotoShareText` in `services/api/src/domain/customerPhotoShare.ts` (share-verb + media-noun text, or bare/near-bare inbound MMS without a question).
- Shared handling in BOTH `/webhooks/twilio` live and regenerate paths: dialog state moves to `inventory_init` (off small_talk), persistent agent context tells future drafts to match the photo before any other follow-up (no discovery restarts), a staff `note` todo is created ("identify the bike, reply with matching units"), and the reply commits to a same-day inventory match in charter voice.
...
## Active-Deal Disposition Closeout Guard (2026-06-11)
- Production incident: Dave Batka +17169982451 — credit application submitted 3 hours earlier (HDFS COA ADF, App ID), open credit-approval task, "Showed / finance needs more info" appointment outcome — auto-archived as `customer_sell_on_own` (0.9 confidence) nine seconds after texting "I am going to take care of the pipes myself". The disposition parser misread an accessory-scope statement as a sell-on-own exit, and the closeout gate had no awareness of live financing.
...
## Cadence Year-Rollover Root Cause (2026-06-11, task #17 closed)
- Root cause of the "June → 2027" parking class: the scheduler tick's `parsePauseUntil` month regex had no ordinal suffix in its day group, so "June 20th" failed the trailing `\b` (digit→"th"), backtracked to a bare "june" match, defaulted to day 1, and the past-date guard rolled a FULL YEAR forward. `bumpCadenceNextDueAt` then parked the lead (Dominik Roehre AND Nicholas Maly, both 2026-06-11, both nextDueAt 2027-06-01T09:00Z). The fingerprint for this class is `{month}-01T09:00:00Z` next year.
- Fixes in `parsePauseUntil`: ordinal-aware day group; modal-"may" guard (mirrors `parseFutureTimeframe` — "I may stop by" is not the month); bare mentions of the CURRENT month resolve to end-of-month, never the same month next year (also applied to `parseFutureTimeframe`'s bare-month branch).
- Future-timeframe day commits: "I signed up on the Harley website for the June 20th thing" used to draw "I'll pause follow-up until june 20" (robotic, no todo). Day/event commitments in the future-timeframe path (live + regen) now get the status-update treatment: confirmation reply, soft-appointment todo, cadence re-anchor to day-after-visit.
- Repair tooling: `POST /internal/worker/cadence-repair/:id` (worker-token gated, never messages the customer) sets nextDueAt/pausedUntil, optionally clears a bogus purchaseTimeframe, optionally adds a staff note todo; logs `cadence_schedule_repair` route outcomes. Repaired: Nicholas Maly (→ Jun 21 2026), Dave Batka (→ pausedUntil alignment + cleared "not interested" timeframe residue), Michael Daniels (→ his genuine next-spring date).
- Audit query for this class: open, non-post-sale, active cadences with nextDueAt > 90 days out — verify each against the lead's stated purchase timeframe before repairing; staff-set resume dates (`toResumeIso` fingerprint: 9am dealer-local) are intentional.
- Gate: `npm run schedule_day_capture:eval` (ordinal + bare-month + modal-may behavioral copies pinned).

## Task Escalation Timers (2026-06-12)
- Competitive parity (dealershipaccelerator.io's 50-minute manager ping): open rep task cards waiting past `TASK_ESCALATION_THRESHOLD_MIN` (default 60) during business hours roll into ONE digest SMS to the manager via `sendInternalSms` — never to customers. Decision logic is pure in `domain/taskEscalation.ts`.
- Guards: overnight tasks start their clock at opening time; notes, future-due reminders, and tasks older than the 48h lookback are excluded; included todos get `escalatedAt` so they never repeat; one digest per `TASK_ESCALATION_COOLDOWN_MIN` (default 120). Kill switch `TASK_ESCALATION_ENABLED=0`. Recipient: `TASK_ESCALATION_NOTIFY_PHONE` else first manager user with a phone.
- Registered in the in-process 60s tick, `WORKER_TICK_DISPATCH`, and the worker `tick-followups` schedule (June 17 flip parity). Gate: `npm run task_escalation:eval`.

## Scheduling-Language Parsing (2026-06-12)
- `parseExactTime` now handles approximate times on round hours ("around 10am", "about 3", "near 9pm") before its catch-all `return null`. Prior behavior dropped the time, so a requested day+time never pinned a slot and bookings fell back to next-available (Chuck Bailey +17163197142: asked "Monday, 15 June around 10am", was offered Saturday Jun 13). Gate: `npm run approximate_time_booking:eval`.
- Sweep convention: `scripts/` one-off sweep over recent customer inbounds runs the REAL parsers (parseRequestedDayTime/parseRequestedDateOnly) and flags day/time language they drop. Run after any scheduling-parser change. 2026-06-12 sweep was clean except the known day-part gap below.
- CLOSED gap (Al Davis +17163059906, 2026-06-06): agent asked "mornings or afternoons?" after offering Saturday; customer replied "Afternoon would be great"; the bare day-part reply did not carry the prior offered day, so no slots were offered (an inventory link went out 2 days later). Two-part root cause: `isShortAckNoReplyText` matched "great" and silently dropped the turn, and a day-part-only reply had no day token so `parseRequestedDayTime` returned null. Fix (2026-06-12): `resolveDayPartOnlyScheduleReply` in workflowRegressionGuards combines the day-part (morning 9-12, afternoon 12-5, evening 5-close, with early/mid/late variants) with the day named in the most recent outbound schedule offer; wired in both the live `/webhooks/twilio` path and `/conversations/:id/regenerate` (route outcomes `day_part_only_schedule_slots` / `day_part_only_schedule_soft_appointment`). If the calendar is unreachable or the window is full, it sets a soft appointment + staff todo instead of bouncing the question back. Day-part words were also added to the `isShortAckNoReplyText` blocker list. Gate: `npm run day_part_only_schedule:eval`.

## Response Latency Grading (2026-06-13)
- `response_latency_audit` measures two clocks per answered customer turn: **agent draft speed** (inbound → first real-time webhook `draft_ai` within 30 min) and **effective** customer-facing latency (inbound → first SENT reply, capped at 7d so cadence re-engagement isn't counted as a "response"). Cadence follow-up drafts are excluded from agent speed so it can't cry wolf.
- **Agent/ops split:** slow real-time drafts are an agent/infra fault and GATE the day (`RELEASE_GATE_AGENT_DRAFT_SLOW_MAX`, default 1). Effective latency is the Suggest-mode staffing lever — reported in every scorecard, never gated. This keeps the clean-day streak an honest agent-readiness signal independent of staff send-speed.
- Prod 7-day baseline (2026-06-13): agent draft median ~30s (excellent); effective median ~186 min, 15% under 5 min, 64% over 1h. The agent is not the bottleneck — human send-time in Suggest mode is. Auto-sending the initial greeting is the highest-leverage lever to move effective latency.
- Wired into `feedback_loop_nightly.sh` before the release gate. Gate: `npm run response_latency:eval`.

## Owned/Comparison Bike Is Not A Request (2026-06-13)
- Todd Herian +15673079691: a Road Glide Special test-ride lead texted "as long as it's a roadglide compared to my current ultra limited" and the agent offered Ultra Limiteds (his trade) for sale. The availability model selector treated the customer's CURRENT/owned bike as the requested model.
- `selectRequestedAvailabilityModelMentions` (workflowRegressionGuards) now drops owned/comparison mentions up front — "my current X", "compared to my X", "than/vs my X", "i ride/own/have a X", "trading in my X" — even on the single-candidate fast path. `normalizeModelText` splits concatenated model names people text as one word (roadglide/streetglide/ultralimited/fatboy/roadking/etc). Gate: `npm run owned_bike_model_guard:eval`.
- BROADER PATTERN (Joe, recurring frustration, filed as a task): when a lead is classified test_ride with a known model, the agent should check THAT model's inventory, say what's in stock, and offer to set up the ride — not send a generic inventory link or a bare options list ending in "I can send more options." Test-ride intent → inventory check + ride offer.

## Answer-Correctness Grading (2026-06-13)
- `answer_correctness_audit` replays real customer turns against the agent's actual next reply. `owned_bike_offered` (customer named a bike as current/owned/trade and the agent offered it — Todd Herian) is GATED at 0 (`RELEASE_GATE_OWNED_BIKE_OFFERED_MAX`); zero false positives on full-history validation. `requested_day_reasked` (customer named a day, agent re-asked which day — Dominik/Chuck class) is REPORTED only until precision rises — conversational text still trips parseRequestedDateOnly. Discipline: validate false-positive rate on full prod history before any check gates; report-don't-gate when noisy (same as effective latency).
- Wired into feedback_loop_nightly.sh before the release gate. Gate: `npm run answer_correctness:eval`.

## Turn Understanding Shadow Soak (2026-06-13, Phase 1)
- `shadowCompareTurnUnderstanding` (index.ts) runs the LLM understanding pass beside the deterministic extractors on every live inbound, fire-and-forget, and logs disagreements (models / owned_bike_surfaced / schedule_presence) to `reports/turn_understanding_shadow/`. Never touches the reply, conv, or store. Reply-path audit stays green.
- ENABLED on americanharley as of 2026-06-13 (`LLM_TURN_UNDERSTANDING_SHADOW=1` in api.env). Costs one extra LLM call per inbound during the soak. Kill switch: set the flag to 0 and redeploy. Nightly disagreement summary: `npm run turn_understanding_shadow:summary`.
- OPS LESSON: the API loads runtime env (`/home/ubuntu/leadrider-runtime/americanharley/api.env`) ONLY via the deploy script, which sources it and runs `pm2 restart --update-env`. A bare `pm2 restart --update-env` from an ssh shell does NOT pick up api.env edits (it keeps the prior launch env). To change a runtime env var: edit api.env, then re-run `bash scripts/deploy_api_lightsail.sh --profile <profile>`.
- Phase 2 (make it the authority for model selection): wait for ~1 week of soak; the disagreement log is the precision measurement and free test set.
