...
import {
  CUSTOMER_DISPOSITION_PARSER_JSON_SCHEMA,
  CUSTOMER_DISPOSITION_EXAMPLES
} from "./customerDispositionPrompt.js";
...
import {
  VOICE_DURABLE_FACTS_JSON_SCHEMA,
  VOICE_DURABLE_FACTS_GUIDELINES,
  VOICE_DURABLE_FACTS_EXAMPLES
} from "./voiceDurableFactsPrompt.js";
...
export async function classifyComplimentWithLLM(args: {
  ...
  const prompt = [
    ...
    "is_compliment=true only when the message is CLEARLY a compliment and nothing more.",
    "is_compliment=false when the message asks ANY question or makes ANY request (coverage, pricing, availability, how-to, scheduling) — even if it also contains positive words. The question deserves a real answer, not a compliment ack.",
    'Example of NOT a compliment: "How does it work? The only thing I would like to do is upgrade my exhaust, is there anything to do with the custom coverage?" — a coverage question; "like" here expresses desire, not praise.',
    "Example of NOT a compliment: \"I'd like to get new pipes installed\" — a service request.",
    'Example of a compliment: "Man those new pipes sound awesome!" — praise with no request.',
    "confidence is 0 to 1.",
    "",
    history.length ? `Recent messages:\n${history.join("\n")}` : "Recent messages: (none)",
    `Message: ${text}`
...
export async function classifyTaskFulfillmentWithLLM(args: {
  ...
  const examples = [
    ...
    // HANDOFF with no substance (Joe, 2026-07-23): a bare escalation promise never fulfills a
    // pricing/financing/availability question — and it is NOT engaged_pending_customer either
    // (the next action still belongs to the DEALER: someone must actually follow up).
    'dealer/sms: "I will have our finance manager follow up with you on that." | task {"task_id":"t9","type":"payments","objective":"Customer asked about financing options and rates for the Street Glide."} -> {"task_id":"t9","fulfilled":false,"confidence":0.95,"evidence":"a handoff promise with no financing details; the customer\'s question is unanswered and the follow-up has not happened","engaged_pending_customer":false,"defer_until":""}',
    'dealer/sms: "Let me get someone from the team to help with pricing, they will reach out shortly." | task {"task_id":"t10","type":"pricing","objective":"Customer asked for the out-the-door price on the 2024 Road Glide."} -> {"task_id":"t10","fulfilled":false,"confidence":0.95,"evidence":"escalation only — no price was given; the ball is still with the dealer","engaged_pending_customer":false,"defer_until":""}',
    // CONTRAST: a handoff that ALSO carries the substantive answer does fulfill the question.
  ...
  const prompt = [
    ...
    "- A HANDOFF/escalation reply ('I'll have someone follow up', 'our finance manager will reach out', 'let me get the team on this') with NO substantive detail does NOT fulfill a pricing/financing/availability/question task — the customer's question is still unanswered. It is also NOT engaged_pending_customer: the next action still belongs to the DEALER (someone must actually follow up), not the customer. Only when the reply ALSO carries the actual answer (a real price, rate, term, or availability fact) is the question fulfilled.",
...
export type PricingPaymentsIntentParse = {
  ...
  // TRUE only when the customer explicitly asks for payment numbers/an estimate this turn
  // ("what would my payment be", "run the numbers", "how much a month"). A customer merely
  // VOLUNTEERING trade/down context ("I have a 2010 Sportster and 3k to put down") is intent
  // "payments" but asksForPaymentEstimate=false — the agent should gather trade/down details,
  // not fire the payment calculator (Joe ruling 2026-07-09, Ryan Tower +15857278545).
...
const PRICING_PAYMENTS_INTENT_PARSER_JSON_SCHEMA: { [key: string]: unknown } = {
  ...
  properties: {
    ...
    asks_for_payment_estimate: { type: "boolean" },
...
const INVENTORY_ENTITY_PARSER_JSON_SCHEMA: { [key: string]: unknown } = {
  ...
  properties: {
    ...
    is_active_unit_clarification: { type: "boolean" },
...
export async function parseAppointmentTimingWithLLM(args: {
  ...
  const examples = [
    ...
    'input: "Customer: are you guys available anytime later on the day? I don\'t think I\'ll be out until after 3 tomorrow" output: {"intent":"ask_for_times","explicit_request":true,"requested":{"day":"tomorrow","time_text":"after 3","time_window":"range"},"reference":"none","normalized_text":"tomorrow after 3","confidence":0.95}',
...
export async function parseCustomerAckActionWithLLM(args: {
  ...
  const examples = [
    ...
    'input: "Customer: I will try to be in there by 9:30" history: "out: No worries—we open up at 9am on Saturday, what time were you thinking on coming in?" output: {"action":"confirm_proposed_appointment","explicit_action":true,"should_reply":true,"should_book":true,"requested":{"day":"saturday","time_text":"9:30 AM","time_window":"exact"},"reference":"last_outbound","normalized_text":"saturday 9:30 AM","confidence":0.9}',
    'input: "Customer: probably around 10" history: "out: What time works for you to come in on Monday?" output: {"action":"confirm_proposed_appointment","explicit_action":true,"should_reply":true,"should_book":true,"requested":{"day":"monday","time_text":"10:00 AM","time_window":"exact"},"reference":"last_outbound","normalized_text":"monday 10:00 AM","confidence":0.9}',
    ...
    'input: "Customer: are you guys available anytime later on the day? I don\'t think I\'ll be out until after 3 tomorrow" history: "out: Want to set up a time to swing by and take a look?" output: {"action":"ask_for_available_times","explicit_action":true,"should_reply":true,"should_book":false,"requested":{"day":"tomorrow","time_text":"after 3","time_window":"range"},"reference":"none","normalized_text":"tomorrow after 3","confidence":0.95}',
    ...
    // NAMING YOUR OWN SHIFT IS THE CUSTOMER COMING BACK — not a sign-off, not a promise of a time.
    // +17169400722, 2026-08-06: Scott voicemailed a live credit-app deal ("call back, or text if
    // easier"); "Hey buddy just got up I work over nights" got SILENCE. Over 6 runs on that exact
    // turn the parser flipped no_response_needed / customer_will_provide_time / neutral_ack — two
    // of the three are silence terminals, so an unlucky sample loses the reply. Pin the REPLY.
    'input: "Customer: Hey buddy just got up I work over nights" history: "out: Hey Chris, this is Scott from American Harley-Davidson. I was texting you back and forth this morning about the credit application. Figured I\'d give you a call to go over some information — if you could give me a callback at this number. If text is easier, just let me know." output: {"action":"neutral_ack","explicit_action":true,"should_reply":true,"should_book":false,"requested":{"day":"","time_text":"","time_window":"unknown"},"reference":"last_outbound","normalized_text":"customer answers the outreach and says he works overnights","confidence":0.9}',
    'input: "Customer: Just woke up, I work third shift" history: "out: Tried you on the phone — what is the best way to reach you?" output: {"action":"neutral_ack","explicit_action":true,"should_reply":true,"should_book":false,"requested":{"day":"","time_text":"","time_window":"unknown"},"reference":"last_outbound","normalized_text":"customer answers the outreach and says he works third shift","confidence":0.9}',
    'input: "Customer: sorry just seeing this, I work nights" history: "out: Just following up on the numbers we went over — let me know if you want me to keep going." output: {"action":"neutral_ack","explicit_action":true,"should_reply":true,"should_book":false,"requested":{"day":"","time_text":"","time_window":"unknown"},"reference":"last_outbound","normalized_text":"customer re-engages late and says he works nights","confidence":0.88}',
    // CONTRAST: the same shift talk WITH an explicit promise to come back with a time really is
    // customer_will_provide_time. The difference is the promise, never the mention of hours.
    'input: "Customer: I work nights so let me look at my schedule and I\'ll text you a time" history: "out: What day works for you to come in?" output: {"action":"customer_will_provide_time","explicit_action":true,"should_reply":false,"should_book":false,"requested":{"day":"","time_text":"","time_window":"unknown"},"reference":"none","normalized_text":"customer will text a time once he checks his schedule","confidence":0.94}'
  ];
  ...
  const prompt = [
    ...
    "- A customer who answers our outreach by NAMING THEIR OWN WORK SCHEDULE — the shift they work or the hours they are awake (\"just got up, I work over nights\", \"sorry just seeing this, I work third shift\", \"I'm on days this week, free after 4\") — is RE-ENGAGING, not signing off: set neutral_ack with should_reply=true. They are telling us when we can reach them, which is an opening, and the thread is usually mid-deal. Do NOT set no_response_needed — nothing was concluded — and do NOT set customer_will_provide_time unless they actually PROMISED to come back with a day or time; merely naming their shift is not a promise. This rule is ONLY about a stated work schedule or waking hours; it does not extend to other self-explanations, which follow the normal sign-off rules.",
    ...
    "- If the dealer's last message ASKED what time the customer wants to come in / be scheduled ON A SPECIFIC DAY (e.g. \"we open at 9 on Saturday, what time were you thinking?\", \"what time works for you on Monday?\") and the customer answers with a concrete clock time (\"9:30\", \"I'll try to be in there by 9:30\", \"probably around 10\"), they are ANSWERING that scheduling question: set confirm_proposed_appointment with should_book=true, carry requested.day from the dealer's question, and put the clock time in requested.time_text. This is DISTINCT from an unprompted en-route ETA with no preceding dealer scheduling question (\"on my way, be there by 5:30\"), which stays provide_arrival_window / should_book=false.",
    ...
    "- An OPEN-ENDED bound (\"after 3\", \"before noon\", \"not until after 3\", \"later in the day\") is NOT a concrete slot confirmation: keep the bound phrase in requested.time_text, set time_window \"range\", and never set should_book=true for a bound-only time — the customer is telling you when they CAN'T come, asking what's open, not confirming a slot.",
    "- confidence is 0 to 1.",
    "",
    `Known lead info: ${JSON.stringify({
      model: lead?.vehicle?.model ?? lead?.vehicle?.description ?? null,
...
export async function parseTurnUnderstandingWithLLM(args: {
  ...
  const examples = [
    ...
    // TRADE vs BUY in ONE message: the WANTED bike -> requested_models; the TRADE bike -> owned_or_trade_model (NEVER requested), even in the same sentence.
    'input: "how much for a road glide? also wanna trade in my street bob" output: {"requested_models":[{"family":"Road Glide","trim":"","confidence":0.9}],"owned_or_trade_model":{"family":"Street Bob","trim":""},"requested_schedule":{"day_label":"","time_text":"","window":"unknown","is_commitment":false,"is_event":false},"primary_intent":"trade","flags":{"is_opt_out":false,"is_wrong_number":false,"is_comparison":false,"asked_for_photos":false},"confidence":0.9}',
    // CHANGE OF MIND: a bike the customer REJECTS/drops ("forget the X", "not the X anymore", "scratch the X") goes in NEITHER field. The NEW bike they name -> requested_models.
    'input: "forget the fat bob, thinking low rider s now" output: {"requested_models":[{"family":"Low Rider S","trim":"","confidence":0.9}],"owned_or_trade_model":{"family":"","trim":""},"requested_schedule":{"day_label":"","time_text":"","window":"unknown","is_commitment":false,"is_event":false},"primary_intent":"availability","flags":{"is_opt_out":false,"is_wrong_number":false,"is_comparison":false,"asked_for_photos":false},"confidence":0.9}',
    // CHANGE OF MIND, rejected bike is NOT owned: dropping interest in a bike does not mean they own it. requested_models AND owned stay empty.
    'input: "scratch the sportster idea, not really feeling it anymore" output: {"requested_models":[],"owned_or_trade_model":{"family":"","trim":""},"requested_schedule":{"day_label":"","time_text":"","window":"unknown","is_commitment":false,"is_event":false},"primary_intent":"smalltalk","flags":{"is_opt_out":false,"is_wrong_number":false,"is_comparison":false,"asked_for_photos":false},"confidence":0.85}',
    // OWNED/TRADE trim: use only what the customer said — do NOT invent a trim. "my 2013 sporty" -> family "Sportster", trim "" (never fabricate "883"/"1200").
    ...
    // SCHEDULE relevance: "running late but STILL COMING for the <existing time>" — the 3pm is their ALREADY-booked slot, not a new proposal. requested_schedule NULL.
    'input: "running 20 min behind, stuck at work but still coming for the 3pm walkthrough" output: {"requested_models":[],"owned_or_trade_model":{"family":"","trim":""},"requested_schedule":{"day_label":"","time_text":"","window":"unknown","is_commitment":false,"is_event":false},"primary_intent":"smalltalk","flags":{"is_opt_out":false,"is_wrong_number":false,"is_comparison":false,"asked_for_photos":false},"confidence":0.9}',
    ...
    // Real misses surfaced by the large-corpus run (2026-07-28): finance docs, price negotiation, follow-up prompt.
    'input: "Hey Scott I have both our pay stubs and proof of address" history: "Agent: To get you pre-approved I just need pay stubs and proof of address." output: {"requested_models":[],"owned_or_trade_model":{"family":"","trim":""},"requested_spec":{"spec_text":"","kind":"none","confidence":0},"requested_schedule":{"day_label":"","time_text":"","window":"unknown","is_commitment":false,"is_event":false},"primary_intent":"finance","flags":{"is_opt_out":false,"is_wrong_number":false,"is_comparison":false,"asked_for_photos":false},"confidence":0.9}',
    'input: "How bout 30..." history: "Agent: The 2022 Street Glide is listed at $28,995. Customer: whats your best price" output: {"requested_models":[],"owned_or_trade_model":{"family":"","trim":""},"requested_spec":{"spec_text":"","kind":"none","confidence":0},"requested_schedule":{"day_label":"","time_text":"","window":"unknown","is_commitment":false,"is_event":false},"primary_intent":"pricing","flags":{"is_opt_out":false,"is_wrong_number":false,"is_comparison":false,"asked_for_photos":false},"confidence":0.88}',
    'input: "Hi Thanks for the follow up that never happened" history: "Agent: I\'ll follow up with you about setting up a time to come in." output: {"requested_models":[],"owned_or_trade_model":{"family":"","trim":""},"requested_spec":{"spec_text":"","kind":"none","confidence":0},"requested_schedule":{"day_label":"","time_text":"","window":"unknown","is_commitment":false,"is_event":false},"primary_intent":"scheduling","flags":{"is_opt_out":false,"is_wrong_number":false,"is_comparison":false,"asked_for_photos":false},"confidence":0.85}'
  ];
  ...
  const prompt = [
    ...
    "- TRADE VALUATION: 'what's my X worth', 'cash value on my X', 'trade value for my X', 'what can I get for my X' => X is their OWN bike being appraised => owned_or_trade_model ONLY. X is NOT also a requested_model, even when named specifically or in slang (a bare trade-valuation has requested_models []). A DIFFERENT bike named as wanted in the same message still fills requested_models.",
    ...
    "- requested_schedule is NULL (not a request) for: an EN-ROUTE / ETA update when they are already on the way ('heading out, there in 30', 'running late, be there at 11:30', 'be there in 20', 'pulling in, 10 min out', 'on my way'); a 'running late but STILL COMING for the <existing appt>' note — a time named there ('3pm', 'the 2 o'clock walkthrough') is their ALREADY-BOOKED slot they are confirming, NOT a new time to propose; a PAST-visit recap ('I stopped in today', 'I came by earlier'); or a bare social sign-off after a time is already set ('see you then', 'perfect, see you Saturday'). These are logistics/acknowledgements, not a new day/time to schedule. Rule of thumb: if the customer is ALREADY on the way or ALREADY booked, requested_schedule is NULL — only a NEW proposed/confirmed day or time to come in fills it. A DELAY or ETA DURATION ('15 mins late', '20 min behind', 'running 10 late', 'in 20', '10 min out') is NOT a schedule time — even when the message ends with a question ('still ok?', 'still good?', 'that work?', 'will the tech be there?'); those ask about an existing plan, they do not propose a new time. Only an actual clock time or day the customer offers to COME IN ('at 2', 'friday 5:30', 'saturday', 'tomorrow afternoon') fills requested_schedule.",
    ...
    "- primary_intent (more, from real misses): SUBMITTING or offering FINANCING/credit-app documents (pay stubs, proof of address, W2s, bank statements, license/SSN for the application) is finance — even phrased as a plain statement ('I have both our pay stubs and proof of address'). A PRICE NEGOTIATION or offer ('how bout 30', 'would you take 25', 'meet me at 22k', 'best you can do') is pricing, NOT scheduling. PROMPTING for a promised-but-missing follow-up / callback ('thanks for the follow up that never happened', 'still waiting to hear back', 'you were gonna call me') is scheduling (they want the agent to act on the pending next step) — not 'other'/smalltalk.",
...
export async function parseManualOutboundPromiseWithLLM(args: {
  ...
  const examples = [
    ...
    // Event-conditioned promise WITH an approximate timeframe (Nicholas Braun +17166286477,
    // operator-reported: the call task came due TOMORROW for a trade arriving "probably next
    // week"). Unlike EXAMPLE C (an open-ended watch for a MODEL), this is a promise about a
  ...
  const prompt = [
    ...
    "- due_text: the stated day/date OR approximate timeframe exactly as said (\"Monday\", \"tomorrow\", \"July 25th\", \"next week\", \"couple weeks\"); empty when none stated. An event-conditioned promise (\"when the trade comes in — probably next week\") carries its stated timeframe.",
    "- confidence 0 to 1.",
    "",
    ...examples,
    "",
...
export async function parseCustomerDispositionWithLLM(args: {
  ...
  const history = (args.history ?? []).slice(-6).map(h => `${h.direction}: ${h.body}`);
  const lead = args.lead ?? {};
  const examples = CUSTOMER_DISPOSITION_EXAMPLES;
  ...
  const prompt = [
    ...
    "- Answering OUR qualifying question is ENGAGED, never a closeout: if a recent 'out:' message asked what they are looking for (style, new vs used, budget, timeframe) and this turn supplies any concrete buying criterion - a condition preference ('used', 'new', 'either'), a payment or budget ceiling INCLUDING non-numeric ones ('lowest monthly payments', 'cheapest payment', 'as low as possible'), a style/model family, or a purchase timeframe - return none with explicit_disposition=false, EVEN WHEN the message opens with an apology for a slow reply and EVEN WHEN it explains a money constraint (medical bills, work, kids). An apology is politeness, not a disposition. A money constraint stated AS A SHOPPING CRITERION is qualification data, not a budget stop. Only treat it as defer_no_window when the customer stops shopping and gives NO criteria.",
    "- Budget stop, no continued shopping: if the customer says they generally can't afford anything right now / are stopping the search over money with no continued-shopping signal AND no stated buying criteria, treat as defer_no_window unless they give a clear timeframe (then defer_with_window).",
    "- Chasing OUR silence is never a goodbye: a short prod that WE went quiet or dropped the ball ('Forget about me?', 'Hello?', 'Still there?', 'Did you forget about me?') — especially after an 'out:' message promised a follow-up that never came — is the customer DEMANDING a response, the opposite of stepping back. Return none. Reading it as stepping_back closes the lead on the very customer we already let down.",
    "- Waiting on ANOTHER PERSON'S decision before moving forward ('till my son decides', 'need to talk to my wife first', 'depends what my buddy does') is a dependency pause on a live deal, NOT a closeout — return none with explicit_disposition=false. This is different from the customer deferring on their OWN account with a promise to reach out later ('I'll get a hold of you when I'm ready'), which is still defer_no_window.",
    ...
    "- DESCRIBING the bike you are trading is NOT selling it privately: talking up your own bike - its condition ('it's absolutely flawless', 'mint', 'never dropped'), its colour, its mileage, or the extras you have for it ('I have the tour pack for it', 'the bars are factory 14 inch') - or sending a PHOTO of it and saying 'that's my bike' / 'that's mine' is the customer PRESENTING THEIR TRADE, not announcing they will sell it themselves. Return none with explicit_disposition=false. sell_on_own requires an explicit statement that they will sell it WITHOUT US ('on my own', 'myself', 'privately', 'list it', 'sell it to a buddy'); possessive or descriptive wording alone is NEVER enough. This matters most mid-negotiation, where the customer is describing the trade precisely BECAUSE a deal is being worked - closing the lead there kills a live deal.",
...
export async function parseReservationConfirmWithLLM(args: {
  ...
  const prompt = [
    ...
    "- A deposit the customer makes CONDITIONAL on a number they are still asking us for is a",
    "  PRICING turn, not a reservation: they are naming what would earn the deposit, not asking us",
    "  to hold anything yet. If the same message asks for a price/payment/out-the-door number and",
    "  offers the deposit on an 'if' ('if it's a good number', 'if the price is right', 'depending",
    "  on the out the door'), answer the price question -> not_reserve_now. Only an UNCONDITIONAL",
    "  deposit/hold ask ('can I put a deposit down to hold it') is reserve_now.",
    ...
    '- "whats the out the door price? I will put a deposit if its a good number" -> {"verdict":"not_reserve_now","confidence":0.93}',
    '- "Best price you can do? If the number is right I\'ll leave a deposit today" -> {"verdict":"not_reserve_now","confidence":0.9}',
...
export async function judgeCadenceQualityWithLLM(args: {
  ...
  const prompt = [
    ...
    "  WARMTH COUNTS AS A REASON. A short, genuine POST-SALE congratulations or thank-you (\"Congrats on",
    "  your Tri Glide Ultra! If you need anything, just let me know\") is send_worthy even though it carries",
    "  no new information — that is a salesperson being a person, not a contentless ping. Judged suppress",
    "  on 2026-08-01, it went out anyway, and the customer replied \"thank you so much... you're very",
    "  professional\". Do NOT fail send_worthy purely for 'no new info' when the message is a warm human",
    "  note appropriate to the moment. The empty-ping rule is for SALES nudges that ask for attention",
    "  while offering nothing — not for gratitude.",
    ...
    "  CHECK THE CLAIMS, NOT JUST THE TOPIC. Any concrete assertion — a discount, a price, a payment, an",
    "  availability statement, an arrival date — must be supported by the thread or the facts given above.",
    "  Two real misses both scored good/0.9 by only checking that the message was on-topic: (a) \"Save",
    "  $4,000 off list price\" attached to a USED 2017 unit when that promotion belongs to a different,",
    "  NEW bike — a money claim borrowed from another unit; (b) \"I'm not seeing a Freewheeler available",
    "  right now\" sent two turns after WE told that same customer we were taking one in next week — the",
    "  message contradicts our own last word. A concrete claim you cannot support from what you were",
    "  given is a FABRICATION: state_fit=false and overall=\"hold\", NOT \"good\" and NOT",
    "  \"needs_regenerate\" — rewording a false claim only says it more smoothly. Specific and confident",
    ...
    "  count as quality. Detail is only worth something if it is TRUE and the customer needs it.",
    ...
    '- msg: "Congrats on your Tri Glide Ultra! If you need anything, just let me know." (post-sale) critique: no new information, but this is a warm human note at the right moment, not a sales ping asking for attention — warmth is a reason. -> {"send_worthy":true,"state_fit":true,"tone_ok":true,"disposition_ok":true,"overall":"good","confidence":0.85,"reason":"warm post-sale note at the right moment","steering":""}',
    '- msg: "quick update on the Breakout: Save $4,000 off list price" where the thread shows a USED 2017 unit and that promotion belongs to a new bike. critique: it LOOKS concrete, which is the trap — the money claim is borrowed from a different unit and nothing given supports it; rewording a false claim only says it more smoothly. -> {"send_worthy":true,"state_fit":false,"tone_ok":true,"disposition_ok":true,"overall":"hold","confidence":0.9,"reason":"discount claim not supported for this unit","steering":""}',
...
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":true,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.97}`,
    `EXAMPLE B
inbound: "I want to stay under $500/month."
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":true,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":true,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.98}`,
    `EXAMPLE C
inbound: "I have $2,500 down and want under $500/mo."
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":true,"asks_down_payment":true,"asks_apr_or_term":false,"asks_for_payment_estimate":true,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.98}`,
    `EXAMPLE D
inbound: "Can you run it for 72 months?"
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":true,"asks_for_payment_estimate":true,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.97}`,
    `EXAMPLE D2
inbound: "What is the longest term I can go with on the loan?"
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":true,"asks_for_payment_estimate":true,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.98}`,
    `EXAMPLE E
inbound: "I don't want to put anything down."
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":true,"asks_apr_or_term":false,"asks_for_payment_estimate":true,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.96}`,
    `EXAMPLE F
inbound: "Any deals or finance specials right now?"
output: {"intent":"pricing","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.95}`,
    `EXAMPLE G
inbound: "What is your best out-the-door price?"
output: {"intent":"pricing","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.97}`,
    `EXAMPLE H
inbound: "Do you have any black street glides in stock?"
output: {"intent":"none","explicit_request":false,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.96}`,
    `EXAMPLE I
inbound: "Can I come in Wednesday at 1?"
output: {"intent":"none","explicit_request":false,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.97}`,
    `EXAMPLE J
inbound: "Before I come in, what do I need to bring for financing?"
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.94}`,
    `EXAMPLE K
inbound: "I'm already approved through my credit union."
output: {"intent":"none","explicit_request":false,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.96}`,
    `EXAMPLE L
inbound: "I can put down 5000"
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":true,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.94}`,
    `EXAMPLE M
inbound: "How much to switch my headlight bulb to LED?"
output: {"intent":"none","explicit_request":false,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.95}`,
    `EXAMPLE N
inbound: "If financing approved at Buffalo Harley is it good at your store?"
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":true,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.98}`,
    `EXAMPLE O
...
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":true,"asks_third_party_purchase_facilitation":true,"confidence":0.98}`,
    `EXAMPLE O3
inbound: "Can your store handle the paperwork if I buy a bike from a private seller?"
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":true,"confidence":0.96}`,
    `EXAMPLE O4
...
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.96}`,
    `EXAMPLE P
inbound: "Did G run the numbers yet?"
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":false,"asks_apr_or_term":false,"asks_for_payment_estimate":true,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.92}`,
    `EXAMPLE Q
inbound: "Awesome thank you I have a 2010 sportster and 3k cash to put down as well"
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":true,"asks_apr_or_term":false,"asks_for_payment_estimate":false,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.95}`,
    `EXAMPLE R
inbound: "With 3k down and my Sportster traded in, what would my monthly be on the Street Glide?"
output: {"intent":"payments","explicit_request":true,"asks_monthly_target":false,"asks_down_payment":true,"asks_apr_or_term":false,"asks_for_payment_estimate":true,"asks_external_approval_transfer":false,"asks_rider_to_rider_financing":false,"asks_third_party_purchase_facilitation":false,"confidence":0.98}`
  ];
  const prompt = [
    ...
    "- asks_for_payment_estimate=true ONLY when the customer explicitly asks for payment numbers or an estimate this turn ('what would my payment be', 'run the numbers', 'how much a month', term/APR structuring questions).",
    "- A customer merely VOLUNTEERING a trade or down payment with no ask ('I have a 2010 sportster and 3k to put down') => asks_for_payment_estimate=false — the store should gather trade/down details, not compute numbers.",
  ...
  return {
    ...
    asksForPaymentEstimate: !!parsed.asks_for_payment_estimate,
...
export async function parseInboundReplyActionWithLLM(args: {
  ...
  const prompt = [
    ...
    "",
    "Actions:",
    "- dealer_location_question: customer asks for the dealership's physical location/address or where the store is (\"where are you\", \"what's your address\", \"where is this located\"). It is NOT for a question about whether a UNIT is at the store, and NOT for a customer who is GIVING us an address.",
    ...
    "- \"Is the bike ON LOCATION / on site / on the lot / here / in stock / do you have it there?\" asks whether a UNIT is physically at the store. That is an AVAILABILITY question, not a request for our address — choose none and let the normal availability path answer it. The tell: they are asking about the BIKE's presence, not about where WE are. This is different from \"where is this bike located\" (which store holds it), which stays dealer_location_question.",
    "- A customer who SUPPLIES an address — their own street, town, or zip, especially right after we asked them for it — is ANSWERING us, not asking where we are. Choose none. Never answer a supplied address by reciting the dealership's address.",
...
export async function parseInventoryEntitiesWithLLM(args: {
  ...
  const lead = args.lead ?? {};
  const voiceExamples = [
    'input: "Customer: do you have any black street glides in stock?" output: {"target_type":"color_model","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Street Glide","year":0,"year_min":0,"year_max":0,"color":"black","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.97}',
    'input: "Customer: looking for a 2026 road glide limited in vivid black" output: {"target_type":"color_model","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Road Glide Limited","year":2026,"year_min":0,"year_max":0,"color":"vivid black","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.97}',
    'input: "Customer: Very interested in thw T10-26 street glide !!" output: {"target_type":"stock_id","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Street Glide","year":2026,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"T10-26","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.97}',
    'input: "Customer: Idk actually I should have given his number.  Itz 716-713-8288. His name is Steve" output: {"target_type":"none","is_availability_question":false,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.95}',
    'input: "Customer: call him at (716) 713-8288" output: {"target_type":"none","is_availability_question":false,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.95}',
    'input: "Customer: do you have the 26 heritage classic in brilliant red?" output: {"target_type":"color_model","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Heritage Classic","year":2026,"year_min":0,"year_max":0,"color":"brilliant red","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.97}',
    'input: "Customer: Road King, any Street Glide, OR Large CC Pan America, would be great." output: {"target_type":"alternate_request","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":true,"model":"Road King","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.94}',
    'input: "Customer: Is this available as well? [MMS image attachment]" output: {"target_type":"image_reference","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.94}',
    'input: "Customer: any pre-owned street glides around 13 to 14 thousand?" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Street Glide","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"used","min_price":13000,"max_price":14000,"monthly_budget":0,"down_payment":0,"confidence":0.96}',
    'input: "Customer: Ken lives in Silver Creek and is looking at a 2026 Street Glide 3 Limited in Iron Horse with Chrome trim. Trade is a 2015 Tri Glide." output: {"target_type":"color_model","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Street Glide 3 Limited","year":2026,"year_min":0,"year_max":0,"color":"Iron Horse","trim":"Chrome trim","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.94}',
    'input: "Customer: how about a tri glide instead?" output: {"target_type":"alternate_request","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Tri Glide","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.95}',
    'input: "Customer: how about a triglycerides instead?" output: {"target_type":"alternate_request","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Tri Glide","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.93}',
    'input: "Customer: if i can stay around 500 a month with 2500 down id do it" output: {"target_type":"none","is_availability_question":false,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":500,"down_payment":2500,"confidence":0.95}',
    'input: "Customer: anything between 2021 and 2023 under 20k?" output: {"target_type":"generic_inventory","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":2021,"year_max":2023,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":20000,"monthly_budget":0,"down_payment":0,"confidence":0.94}',
    'input: "Customer: Any early 2000s lowriders?" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Low Rider","year":0,"year_min":2000,"year_max":2005,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.95}',
    'input: "Customer: do you have any street glides 2015 or older?" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Street Glide","year":0,"year_min":0,"year_max":2015,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.94}',
    'input: "Customer: im after a black trim cvo street glide st" output: {"target_type":"color_model","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"CVO Street Glide ST","year":0,"year_min":0,"year_max":0,"color":"","trim":"black trim","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.93}',
    'input: "Customer: tuesday at 4 works for me" output: {"target_type":"none","is_availability_question":false,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.92}',
    'input: "Customer: Is it a 15 or 16? (recent history: we quoted an approval on the 2015 Road King and earlier mentioned the 2016 Road King)" output: {"target_type":"none","is_availability_question":false,"is_active_unit_clarification":true,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.9}',
    'input: "Customer: is that the 2015? (recent history: we quoted a specific Road King this thread)" output: {"target_type":"none","is_availability_question":false,"is_active_unit_clarification":true,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.9}',
    // --- Harley domain comprehension (Joe glossary Phase 2, 2026-07-24): engine/era/fuel/tank/anniversary/generation -> YEAR BAND on existing slots ---
    'input: "Customer: got any twin cam road kings?" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Road King","year":0,"year_min":1999,"year_max":2016,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.9}',
    'input: "Customer: looking for a shovelhead electra glide" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Electra Glide","year":0,"year_min":1966,"year_max":1984,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.88}',
    'input: "Customer: any milwaukee eight street glides?" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Street Glide","year":0,"year_min":2017,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.9}',
    'input: "Customer: do you have a carbureted softail" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Softail","year":0,"year_min":0,"year_max":2000,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.86}',
    'input: "Customer: after a fuel injected road king" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Road King","year":0,"year_min":2002,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.86}',
    'input: "Customer: any 6 gallon tank street glides?" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Street Glide","year":0,"year_min":2008,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.85}',
    'input: "Customer: 120th anniversary street glide" output: {"target_type":"exact_year_model","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Street Glide","year":2023,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.9}',
    'input: "Customer: you got a gen 2 street glide?" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Street Glide","year":0,"year_min":2024,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.85}',
    'input: "Customer: looking for a mono shock softail" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Softail","year":0,"year_min":2018,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.85}',
    // --- Physical SPEC search -> NARROW to a group, never resolve one bike or fire a watch (leave model empty) ---
    'input: "Customer: got anything with a fat 240 rear tire?" output: {"target_type":"generic_inventory","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.82}',
    'input: "Customer: a v-rod with a wide tire" output: {"target_type":"generic_inventory","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.8}',
    // --- Police variant -> combines with base model; ex-police -> used ---
    'input: "Customer: do you have an electra glide police?" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Electra Glide Police","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.86}',
    'input: "Customer: looking for an ex-police road king" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Road King Police","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"used","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.85}',
    // --- Installed kit/accessory is a MODIFICATION -> base model still resolves, kit is never the model ---
    'input: "Customer: interested in a Fat Boy with the Night Stalker kit" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Fat Boy","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.85}',
    // --- Buell / Aermacchi = a separate MAKE -> never Harley stock (leave model empty, target_type none); trade context for the appraiser ---
    'input: "Customer: I have a Buell Ulysses I want to trade in" output: {"target_type":"none","is_availability_question":false,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"used","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.88}',
    'input: "Customer: do you have any buell blasts?" output: {"target_type":"none","is_availability_question":false,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.85}',
    'input: "Customer: got an aermacchi sprint 350?" output: {"target_type":"none","is_availability_question":false,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.83}',
    // --- Ambiguous terms -> do NOT hard-resolve one specific bike (clarify downstream) ---
    'input: "Customer: you got any 883s?" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Sportster 883","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.72}',
    'input: "Customer: looking for a low rider" output: {"target_type":"model_only","is_availability_question":true,"is_active_unit_clarification":false,"is_test_ride_context":false,"model":"Low Rider","year":0,"year_min":0,"year_max":0,"color":"","trim":"","stock_id":"","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0,"confidence":0.72}'
  ];
  const prompt = [
    ...
    "- is_availability_question: true when the customer is asking about availability, options, in-stock status, or expressing inventory interest in a concrete target; false for pure scheduling, thanks, humor, finance-only, paperwork updates, or a clarification about the specific unit already under discussion.",
    "- is_active_unit_clarification: true when the customer is asking us to CONFIRM or DISAMBIGUATE an attribute (especially the model YEAR, or which of two units we quoted) of the SAME vehicle already under discussion in this thread — e.g. after we quoted/mentioned a specific bike, they ask \"is it a 15 or 16?\", \"is that the 2015?\", \"which one is this?\". This is NOT a request to search inventory, so when it is true also set is_availability_question=false. false otherwise.",
    ...
    "- confidence is 0..1.",
    "",
    "Harley domain knowledge (Joe glossary): combine an engine/era/fuel/tank/edition/generation term WITH the named model to fill year_min/year_max (a BAND) or year (a single year). The term NARROWS the year — it never resolves a model by itself, never fires a broad search, never fabricates a color/trim/code. Open-ended '2017+' => year_min=2017, year_max=0; 'up to 2007' => year_min=0, year_max=2007.",
    "- Engine era -> year band (paired with the model): flathead 1929-1973; knucklehead/knuckle 1936-1947; panhead/pan 1948-1965; shovelhead/shovel 1966-1984 (generator/pan-shovel/slab-side shovel = 1966-1969); ironhead 1957-1985; evo/evolution/blockhead 1984-1999 (evo sportster 1986-2022); twin cam/twincam/tc 1999-2016 (tc88 1999-2006, tc96 2007-2016, tc103 2010-2016, tc110 2009-2016); m8/milwaukee eight/milwaukee-8/wafflehead 2017+ (107/114/117 = 2017+, 121 = 2023+); revolution 2001-2017; revolution max/rev max/revmax 2021+ (975 = 2022+ Nightster, 1250 = 2021+ Sportster S / Pan America).",
    "- Fuel delivery -> year band (family-specific, narrows only): carbureted/carb {model} = older, fuel injected/injected/EFI {model} = newer. Cutovers: Touring carb <=2001 / EFI 2002+; Softail <=2000 / 2001+; Dyna <=2003 / 2006+; Sportster <=2006 / 2007+.",
    "- Tank spec -> year (Touring): 6 gallon/6 gal/six gallon tank => 2008+ (year_min=2008); 5 gallon/5 gal tank => up to 2007 (year_max=2007).",
    "- Anniversary edition -> single year = 1903 + N: 90th 1993, 95th 1998, 100th 2003, 105th 2008, 110th 2013, 115th 2018, 120th 2023, 125th 2028. '120th anniversary Street Glide' => model=Street Glide, year=2023. Bare 'anniversary' with no number => do not guess a year.",
    "- Generation/redesign (THIS dealer's authoritative definition) -> year band: 'gen 2 touring / gen 2 / 2nd gen touring' = the new-design touring generation, staged: 2023+ CVO Street Glide & CVO Road Glide, 2024+ non-CVO Street Glide & Road Glide, 2026+ the rest of Touring + Trikes (Electra Glide/Ultra, Road King, Tri Glide, Freewheeler). So 'gen 2 street glide' => year_min=2024; 'gen 2 cvo road glide' => year_min=2023. 'project rushmore/rushmore' => 2014+ touring. 'mono shock/monoshock softail' => 2018+ Softail; 'twin shock/hidden shock softail' => up to 2017 Softail.",
    "- Police/authority variant -> combines with the base model: 'electra glide police' => Electra Glide Police, 'road king police' => Road King Police, 'road glide police' => Road Glide Police, 'dyna defender' => Dyna Defender. Bare 'police' alone => do not guess the base (leave model empty). 'ex-police'/'former police'/'retired police' => condition=used.",
    "- Installed kit/accessory/custom is a MODIFICATION, never a different model: the BASE model still resolves. 'Fat Boy with the Night Stalker kit' => model=Fat Boy (NEVER 'Night Stalker'). 'Stage IV', 'big bore kit', aftermarket exhaust/pipes => keep the base model; the kit must never become the model or blank the model. Do not fabricate the kit's contents.",
    "- Physical SPEC search (a feature, not a model name) NARROWS to a GROUP -> do NOT resolve one bike and do NOT set a single model: 'wide tire/fat tire/fat rear/wide rear/240/240mm', 'v-rod with a wide tire', 'forward controls', 'ape hangers' => leave model empty, target_type=generic_inventory. A spec term must never resolve a single model, fire a watch, or claim specificity. (A named family like 'Softail' may stay as the family; a spec never upgrades it to one specific model.)",
    "- Buell and Aermacchi are a SEPARATE MAKE, never Harley stock: 'buell', 'blast', 'firebolt', 'ulysses', 'thunderbolt', 'cyclone', '1125R/1125CR', 'S1/X1/XB9/XB12/CityX', and 'aermacchi/sprint/rapido/baja/SS-*/SX-*' => leave model EMPTY (never an HD model), target_type=none. These are trade-in makes for the appraiser, not HD inventory. Never fabricate an HD model for them. ('lightning' only means Buell in Buell context.)",
    "- Ambiguous terms -> do NOT hard-resolve to one specific bike (a clarify happens downstream): '883' (Iron 883 vs older Sportster 883) => model 'Sportster 883', NEVER 'Iron 883'; '1200' (several Sportster 1200s); 'iron' (Iron 883 model vs Ironhead engine); 'low rider' (base vs S vs ST) => model 'Low Rider', NEVER 'Low Rider S'; 'bobber', 'naked', 'standard', 'cafe racer', 'chopper', 'hardtail'. A bare displacement number (107/114/etc) narrows year/trim, it never picks a model alone.",
    "- Vintage/collector editions (recognize only; collector/trade context; never new stock; never fabricate a code/spec/year): confederate (1977), hydra-glide (1949-57 vintage vs the 2024 Hydra-Glide Revival -> clarify), duo-glide (1958-64), sturgis (1980 FXB vs 1991 FXDB -> clarify), liberty edition (1976 vs 1986 vs 2026 -> clarify), vr1000, xr750, xr1000. Do not assign a fabricated current-year unit; leave year 0 unless the customer states one.",
    "",
  ...
  return {
    targetType: validTargetTypes.includes(targetTypeRaw as NonNullable<InventoryEntityParse["targetType"]>)
      ...
      : "none",
    isAvailabilityQuestion: !!parsed.is_active_unit_clarification
      ? false
      : !!parsed.is_availability_question,
    isActiveUnitClarification: !!parsed.is_active_unit_clarification,
...
export type VoiceDurableFactsParse = {
  ...
  preferences: string[];
  blockers: string[];
  // The concrete next step agreed on the call — the PLAN, extracted the same way the
  // quote facts are, so the rep never has to log it by hand (Joe, 2026-07-19).
  // Optional on the type so pre-existing fixture constructions stay valid; the parser
  // always emits them (strict schema).
  nextStepOwner?: "customer" | "staff" | "none";
  nextStepAction?: string;
  nextStepDueText?: string;
  nextStepIsVisit?: boolean;
...
export async function parseVoiceDurableFactsWithLLM(args: {
  ...
  const examples = VOICE_DURABLE_FACTS_EXAMPLES;
  const prompt = [
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object") return null;
    const nextStepOwnerRaw = String(parsed.next_step_owner ?? "none").trim().toLowerCase();
    return {
      ...
      blockers: Array.isArray(parsed.blockers)
        ...
        : [],
      nextStepOwner:
        nextStepOwnerRaw === "customer" || nextStepOwnerRaw === "staff" ? nextStepOwnerRaw : "none",
      nextStepAction: String(parsed.next_step_action ?? "").trim(),
      nextStepDueText: String(parsed.next_step_due_text ?? "").trim(),
      nextStepIsVisit: parsed.next_step_is_visit === true,
      nextStepConfidence: Number(parsed.next_step_confidence ?? 0),
      confidence: Number(parsed.confidence ?? 0)
    };
  } catch {
    return null;
...
// ---------------------------------------------------------------------------
// Merged unified slot parser — the parser-consolidation slice
// (docs/comprehension_consolidation_plan.md, 2026-07-02; CUT OVER LIVE
// 2026-07-14, PR #204). ONE structured call carries all three jobs the legacy
// path runs as parallel sub-calls (semantic slots + trade payoff + trade
// target value). Serves the SMS live + regenerate paths when
// LLM_UNIFIED_SLOT_MERGED_LIVE=1 (via parseUnifiedSemanticSlotsWithLLM, with
// applyMergedWatchRelevanceGuard on the result); output is normalized by the
// SAME shared guard rails as the legacy sub-parsers.
// NOTE: the rules/examples below are still a COPY of the three legacy prompts
// (the email path runs those directly) — if you edit a legacy parser's
// rules/examples, mirror it here (and vice versa) until the email-lane cutover
// lets the legacy copies be deleted. Pinned by the mirror tripwire in
// unified_slots_merged_shadow:eval.
// ---------------------------------------------------------------------------
...
export async function parseUnifiedSemanticSlotsMergedWithLLM(args: {
  ...
  const semanticExamples = [
    ...
    // NARRATIVE EXTRACTION (2026-07-13 shadow review, merged-only like the anti-fabrication block):
    // a long story turn still names a real bike. Anti-fabrication applies to NOISE (URLs, App IDs,
    // intake-blob metadata) — not to a customer telling the story of the bike they want. Shadow
    // miss: "gator harley... 2005 cvo... sold the bike out from under me" extracted NOTHING while
    // legacy correctly got CVO/2005/used. (Legacy also emitted min_price=10000 there, which appears
    // NOWHERE in the text — this example deliberately pins min_price=0: extract, don't invent.)
    'input: "Customer: gator harley of leesburg, florida . 2005 cvo. got tied up at work and they sold the bike out of under me. when i got a call at 3:30 and we talked about the numbers everything was good to go and i said i d be in the morning and i would left a deposit. i would have no problem leaving money on it" output: {"watch_action":"set_watch","watch":{"model":"CVO","year":"2005","year_min":0,"year_max":0,"color":"","condition":"used","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0},"department_intent":"none","contact_preference_intent":"none","media_intent":"none","service_records_intent":false,"confidence":0.9}',
    // Catalog display form ("Forty-Eight", hyphenated) + explicit year range goes in year_min/max, never
    // the year string. Production turn pinned VERBATIM (incl. the out-of-area tail — where the customer
    // is from does not cancel a sourcing request; set_watch is still the right action).
    'input: "Customer: Hello, I m looking for Harley Davidson Forty Eight 2016-2018 model, Specifically Corona Yellow Colour, I only need this colour, I m from India" output: {"watch_action":"set_watch","watch":{"model":"Forty-Eight","year":"","year_min":2016,"year_max":2018,"color":"corona yellow","condition":"unknown","min_price":0,"max_price":0,"monthly_budget":0,"down_payment":0},"department_intent":"none","contact_preference_intent":"none","media_intent":"none","service_records_intent":false,"confidence":0.95}'
  ];
  ...
  const prompt = [
    ...
    "- Anti-fabrication is about NOISE, not length: a long narrative/story message still counts when the customer NAMES the bike they want (e.g. one that got sold out from under them at another dealer). Extract model/year/condition from the named bike; a pre-2020 named year implies condition=used. Do not let the noise rules blank a bike the customer actually described.",
    "- A sourcing request with a specific hard requirement counts as asking to be notified when one becomes available: 'I'm looking for X, I only need this colour' or 'has to be the 2016-2018 model' means find-one-and-tell-me, so watch_action=set_watch. Where the customer is from does not cancel the request.",
    "- Normalize watch.model to its catalog display form (e.g. 'Forty Eight' -> 'Forty-Eight', 'forty-eight' -> 'Forty-Eight').",
    "- confidence is 0..1.",
    "",
    "Examples:",
    ...semanticExamples,
...
// NOTE (2026-07-15): the live shadow-compare runners that produced the
// unified_slots_shadow JSONL corpus were retired after the merged-parser
// cutover shipped (PR #204) — the 12-day / 2,413-turn soak reached its verdict
// (99.4% decision-scoped agreement) and each further comparison cost an extra
// LLM call per turn for evidence already banked. The pure comparator
// (diffUnifiedSlotParse, above) and the historical JSONL + report script are
// KEPT — they're the acceptance harness for the email-lane cutover next.
...
// draft path (same best-effort append contract as appendParserCaptureRecord).
