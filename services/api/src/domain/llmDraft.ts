...
export async function describeUnitEquipmentWithLLM(args: {
  ...
  const prompt = [
    ...
    "- has_fairing: FIXED painted BODYWORK around the headlight (not a see-through shield). fairing_type:",
    "  'batwing' = a fork-mounted frontal fairing that turns WITH the handlebars (Street Glide, Electra Glide);",
    "  'sharknose' = a larger frame-mounted fairing that stays fixed while the bars turn (Road Glide, with its",
    "  distinctive dual headlights); 'unknown' if a fairing is present but you can't tell which.",
    "  A small clear wind deflector on top of a fairing does NOT make has_windshield true — that bike's front",
    "  is a fairing.",
