...
import {
  ...
  buildRoutingDecisionParserPrompt
} from "./routingDecisionParserPrompt.js";
import {
...
export async function parseCustomerAckActionWithLLM(args: {
  ...
  const examples = CUSTOMER_ACK_ACTION_EXEMPLARS;
...
export async function parseVehicleChoiceConfidenceWithLLM(args: {
  ...
  const history = (args.history ?? []).slice(-6).map(h => `${h.direction}: ${h.body}`);
  const prompt = buildVehicleChoiceConfidencePrompt({
    referencedModel: args.referencedModel,
    history,
    text
  });
...
export async function parseRoutingDecisionWithLLM(args: {
  ...
  const prompt = buildRoutingDecisionParserPrompt({
    text,
    history,
    lead: args.lead,
