...
function extractAdfLeadRef(body: string, providerMessageId?: string | null): string {
  const direct =
    extractLineValue(body, "Ref") ||
    extractLineValue(body, "Lead Ref") ||
    extractLineValue(body, "LeadRef") ||
    extractLineValue(body, "Ref #");
  if (direct) return direct.replace(/[^\dA-Za-z_-].*$/, "").trim();
  const bodyMatch = body.match(/\b(?:lead\s*ref|leadref|ref\s*#?|reference)\s*[:#-]?\s*([A-Za-z0-9_-]{3,})/i);
  if (bodyMatch?.[1]) return bodyMatch[1].trim();
  const providerMatch = String(providerMessageId ?? "").match(/\badf[_-]ref[_-]([A-Za-z0-9_-]{3,})\b/i);
...
function buildSoldLeadRefOptions(conv?: ConversationDetail | null): LeadRefOption[] {
  ...
  for (const message of conv?.messages ?? []) {
    ...
    const leadRef = extractAdfLeadRef(body, message.providerMessageId);
    if (!leadRef) continue;
...
const CAMPAIGN_EMAIL_CONTEXT_MARKER = "[Reference Campaign Context]";
...
const CAMPAIGN_EMAIL_CONTEXT_IMAGE_TARGET_PRIORITY: CampaignAssetTarget[] = [
  "web_banner",
  "facebook_post",
  "instagram_post",
  "flyer_8_5x11",
...
function campaignEmailContextPrimaryImage(entry: CampaignEntry): string {
  const generatedAssets = Array.isArray(entry.generatedAssets) ? entry.generatedAssets : [];
  for (const target of CAMPAIGN_EMAIL_CONTEXT_IMAGE_TARGET_PRIORITY) {
    const byTarget = generatedAssets.find(asset => {
      const assetTarget = String(asset?.target ?? "").trim();
      const url = String(asset?.url ?? "").trim();
      return assetTarget === target && looksLikeCampaignImageUrl(url);
    });
    if (byTarget?.url) return String(byTarget.url).trim();
  }
  const finalImageUrl = String(entry.finalImageUrl ?? "").trim();
  if (looksLikeCampaignImageUrl(finalImageUrl)) return finalImageUrl;
  const inspiration = Array.isArray(entry.inspirationImageUrls) ? entry.inspirationImageUrls : [];
  for (const raw of inspiration) {
    const url = String(raw ?? "").trim();
    if (looksLikeCampaignImageUrl(url)) return url;
  }
  ...
  for (const entry of selectedEntries) {
    ...
    contextBlocks.push(
      [
        ...
        refUrls.length
          ? [`Primary campaign image: ${refUrls[0]}`].join("\n")
          : "",
        briefUrls.length
          ? ["Campaign brief files:", ...briefUrls.map((url, idx) => `${idx + 1}. ${url}`)].join("\n")
          : ""
      ]
        .filter(Boolean)
        .join("\n")
  ...
  const campaignEmailContextOptions = useMemo(
    () =>
      campaigns.map(row => ({
        ...
        name: String(row?.name ?? "").trim() || "Untitled campaign",
                  ...
                  <div className="border rounded-lg p-3 bg-white/95 lr-campaign-locker-card space-y-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <button
                        type="button"
                        className="lr-campaign-locker-action lr-campaign-locker-action-secondary"
                        onClick={() => clearEmailContextCampaignFromForm()}
                      >
                        Clear Context
                      </button>
                    </div>
